...
Begin {
    ...
    Function Get-DateTime {
        ...
        # ClientLocal
            ...
            $obj = (Get-Date -Format 'dd.MM.yyyy HH:mm:ss')
        }
    ...
    Function Get-LogFileName {
        ...
        try {
            Test-Path -Path $logshare -ErrorAction Stop | Out-Null
        } catch {

            $ErrorCategory = $_.CategoryInfo.Category
            if ($ErrorCategory -like 'PermissionDenied') {
                #Write-Output $ErrorCategory
                $logshare = $Xml.Configuration.Log | Where-Object { $_.Name -like 'LocalLogFile' } | Select-Object -ExpandProperty 'Share'
            }

        } finally {
            if (!(Test-Path -Path $logshare)) {
                New-Item -Path $logshare -ItemType Directory -Force | Out-Null
            }
        }

        #$obj = "$logshare\$OSName\$env:computername.log"
        $obj = "$logshare\$($env:computername)_ClientHealth.log"

        Write-Output $obj
    }

    function Get-SeverityLogFileName {
        [CmdletBinding()]
        param (
            [Parameter()]
            $Severity
        ...
        $logSeverity = Get-XMLConfigLoggingSeverity
        ...
        switch ($logSeverity) {
            'Error' {
                if ($Severity -ge 3) {
                    #$FileName = $FileName.Replace('ClientHealth', 'ClientHealth_Error')
                    $FileName = $FileName.Replace('ClientHealth', 'ClientHealth_Repaired')
                    $obj = $logFileName.Replace($logFileName.Split('\\')[-1], $FileName)
            ...
            'Warning' {
                if ($Severity -ge 2) {
                    $FileName = $FileName.Replace('ClientHealth', 'ClientHealth_Warning')
                    $obj = $logFileName.Replace($logFileName.Split('\\')[-1], $FileName)
            ...
            Default {
                if ($config) {
                    $LocalShare = $Xml.Configuration.Log | Where-Object { $_.Name -like 'LocalLogFile' } | Select-Object -ExpandProperty 'Share'
                    $obj = "$LocalShare\$($env:computername)_ClientHealth.log"
                } else {
                    $obj = $null
                }
    ...
    Function Get-ServiceUpTime {
        ...
        Try {
            ...
            Return (New-TimeSpan -Start $ServiceStartTime -End (Get-Date -Format 'dd.MM.yyyy HH:mm:ss')).Days
    ...
    Function Out-LogFile {
        ...
        if ($mode -like 'ClientInstallFailed' ) {
        ...
        if ($Mode -like 'Local') {
        ...
        } elseif ($Mode -eq 'ClientReInstalled') {

            $NetLogFile = Get-SeverityLogFileName -Severity 3

            $LogFileName = $NetLogFile.Split('\\')[-1]
            $LogFolder = $NetLogFile.Replace($LogFileName, '')

            foreach ($Message in $text) {
                Write-Log -Message $Message -LogFolder $LogFolder -Log $LogFileName -Severity $Severity
            }
            ...
            $LogFile = Get-SeverityLogFileName -Severity $Severity
        }
        ...
        if ($LogFile) {
            ...
            foreach ($item in $text) {
                ...
                $date = 'date="' + (Get-Date -Format dd.MM.yyyy) + '"'
                ...
                $item + $logblock | Out-File -Encoding utf8 -Append $logFile -Force
            }
...
#>
    ...
    Function Get-ClientVersion {
        ...
        try {
            ...
            #if ($PowerShellVersion -ge 6) {
            $obj = (Get-CimInstance -Namespace 'root\ccm' -ClassName 'SMS_Client' -ErrorAction SilentlyContinue).ClientVersion
            #} else {
    ...
    .DESCRIPTION
    Function to test if local database files are missing from the ConfigMgr client.
    Will tag client for reinstall if less than 7.
    Returns $True if compliant or $False if non-compliant
    ...
    .NOTES
    Returns $True if compliant or $False if non-compliant.
    Non.compliant computers require remediation and will be tagged for ConfigMgr client reinstall.
    #>
    ...
    Function Test-ClientSettingsConfiguration {
        ...
        $ClientSettingsConfig = @(Get-CimInstance -Namespace 'root\ccm\Policy\DefaultMachine\RequestedConfig' -Class CCM_ClientAgentConfig -ErrorAction SilentlyContinue | Where-Object { $_.PolicySource -eq 'CcmTaskSequence' })
        ...
        if ($ClientSettingsConfig.Count -gt 0) {
            ...
            if ($fix -eq 'true') {
                ...
                DO {
                    Get-CimInstance -Namespace 'root\ccm\Policy\DefaultMachine\RequestedConfig' -Class CCM_ClientAgentConfig | Where-Object { $_.PolicySource -eq 'CcmTaskSequence' } | Select-Object -First 1000 | ForEach-Object { Remove-CimInstance -InputObject $_ }

                } Until (!(Get-CimInstance -Namespace 'root\ccm\Policy\DefaultMachine\RequestedConfig' -Class CCM_ClientAgentConfig | Where-Object { $_.PolicySource -eq 'CcmTaskSequence' } | Select-Object -First 1))
    ...
    Function Test-DNSConfiguration {
        ...
        try {
            Get-CimInstance -ClassName MSFT_DNSClientServerAddress -Namespace 'ROOT/StandardCimv2' -ErrorAction SilentlyContinue | Out-Null

        } catch {
            $Message = "Error: $($_.Exception.Message)"
            $LogId = $($MyInvocation.MyCommand.Name)
            $Component = [string]::Format('{0}:{1}', $logID, $($_.InvocationInfo.ScriptLineNumber))
        ...
        if (($OSName -notlike '*Windows 7*') -and ($OSName -notlike '*Server 2008*')) {
            ...
            try {
                ...
                #$DNSServer = Get-CimInstance -ClassName MSFT_DNSClientServerAddress -Namespace 'ROOT/StandardCimv2'
    ...
    Function Test-ConfigMgrClient {
        ...
        if (Get-Service -Name ccmexec -ErrorAction SilentlyContinue) {
            ...
            if ( $reinstall -eq $true) {
                ...
                if ( ($localDB -eq $true) -and (-NOT ($clientInstallProperties -like '*/forceinstall*')) ) {
            ...
            if (Get-Service -Name ccmexec -ErrorAction SilentlyContinue) {
                ...
                Out-LogFile 'ConfigMgr Client installation failed. Agent not detected 5 minutes after triggering installation.' -Mode 'ClientInstall' -Severity 3
            }
        }
    }
    ...
    Function Test-RegistryPol {
        ...
        if ($Days) {
            ...
            try {
                ...
                $Message = 'GPO Cache: Failed to check machine policy age.'
                Out-LogFile -Xml $Xml -Text $Message -Mode 'ClientReInstalled' -Severity 3
        ...
        try {
            ...
            $Message = 'GPO Cache: Failed to check the event log for policy errors.'
            Out-LogFile -Xml $Xml -Text $Message -Mode 'ClientReInstalled' -Severity 3
            Write-Warning $Message
        }
        ...
        if ($RepairReason -ne '') {
            ...
            $Message = "GPO Cache: Broken ($RepairReason)"
            Out-LogFile -Xml $Xml -Text $Message -Mode 'ClientReInstalled' -Severity 3
            ...
            try {
                ...
                $Message = "GPO Cache: Failed to remove the registry file ($($MachineRegistryFile))."
                Out-LogFile -Xml $Xml -Text $Message -Mode 'ClientReInstalled' -Severity 3
                Write-Warning $Message
            } finally {
                ...
                try {
                    ...
                    $Message = 'GPO Cache: Failed to run GPUpdate.'
                    Out-LogFile -Xml $Xml -Text $Message -Mode 'ClientReInstalled' -Severity 3
                    Write-Warning $Message
                }
            }
    ...
    Function Resolve-Client {
        ...
        if ((Test-Path $ClientShare -ErrorAction SilentlyContinue) -eq $true) {
            ...
            Out-LogFile -Xml $xml -Text $text -Severity 2
            Out-LogFile -Xml $xml -Text $text -Mode 'ClientReInstalled' -Severity 3
            Write-Output $text
            ...
            if ($Uninstall -eq $true) {
                Get-Process -Name ccmsetup -ErrorAction SilentlyContinue | Stop-Process -Force
                ...
                do {
                    ...
                    If ($MinutesLeft -le 0) {
                        $text = "Timed out waiting $($WaitMinutes) minutes for ConfigMgr Client Uninstallation processes to complete.  Will not restart the Uninstall again."
                        $Severity = 3

                        Write-Warning $text
                        Out-LogFile -Xml $xml -Text $text -Severity $Severity
    ...
    function Register-CommonDLLs {
        ...
        try {
            Invoke-Command -ScriptBlock $RegisterCommonDLLs
        } catch {
            $Message = "Error: $($_.Exception.Message)"
            $LogId = $($MyInvocation.Invocation.MyCommand).Name
            $Component = [string]::Format('{0}:{1}', $logID, $($_.InvocationInfo.ScriptLineNumber))
    ...
    function Register-DLLFile {
        ...
        try {
            Start-Process -FilePath 'regsvr32.exe' -Args "/s `"$FilePath`"" -Wait -NoNewWindow -PassThru
        } catch {
            $Message = "Error: $($_.Exception.Message)"
            $LogId = $($MyInvocation.Invocation.MyCommand).Name
            $Component = [string]::Format('{0}:{1}', $logID, $($_.InvocationInfo.ScriptLineNumber))
    ...
    function Invoke-WMIRepair {
        ...
        try {
            Invoke-Command -ScriptBlock $RepairWMI -ErrorAction SilentlyContinue
        } catch {
            $Message = "Error: $($_.Exception.Message)"
            $LogId = $($MyInvocation.Invocation.MyCommand).Name
            $Component = [string]::Format('{0}:{1}', $logID, $($_.InvocationInfo.ScriptLineNumber))
    ...
    Function Test-RefreshComplianceState {
        Param (
            $Days = 0,
            [Parameter(Mandatory = $true)]
            $RegistryKey = $Script:CHRegistryPath,
            [Parameter(Mandatory = $true)]
            $Log
        )
        ...
        try {
            ...
            $LastSentEntry = Get-RegistryValue -Path $RegistryKey -Name $RegValueName

            try {
                $Format = 'dd.MM.yyyy HH:mm:ss'
                $LastSent = [DateTime]::ParseExact($LastSentEntry, $Format, $Culture)
            } catch {
            ...
            [datetime]$LastSent = Get-Date #-Format 'dd.MM.yyyy HH:mm:ss'
        ...
        If (($NumberOfDays -le 0) -or ((Get-Random -Maximum $NumberOfDays) -eq 0 )) {
            Try {
                ...
                $LastSent = Get-Date #-Format 'dd.MM.yyyy HH:mm:ss'
        ...
        Set-RegistryValue -Path $RegistryKey -Name $RegValueName -Value $LastSent.ToString('dd.MM.yyyy HH:mm:ss', $Culture)
                ...
                if ($ServiceUptime -ge $Uptime) {
                    try {
                        ...
                        While ((Get-Process -Name WUSA, wuauclt, setup, TrustedInstaller, msiexec, TiWorker, ccmsetup -ErrorAction SilentlyContinue).Count -gt 0) {
                            ...
                            If ($MinutesLeft -le 0) {
                                $text = "Timed out waiting $($WaitMinutes) minutes for installation processes to complete.  Will not restart the $($Name) service."
                                $Severity = 3
    ...
    Function Test-CCMSoftwareDistribution {
        ...
        Get-CimInstance -Class CCM_SoftwareDistributionClientConfig
    }
    ...
    Function Get-LastReboot {
        ...
        if ($execute -eq $true) {
            ...
            if ($uptime.TotalDays -lt $maxRebootDays) {
                $text = 'Last boot time: ' + $lastBootTime.ToString('dd.MM.yyyy HH:mm:ss') + ': OK'
                Write-Output $text
            } elseif (($uptime.TotalDays -ge $maxRebootDays) -and (Get-XMLConfigRebootApplicationEnable -eq $true)) {
                $text = 'Last boot time: ' + $lastBootTime.ToString('dd.MM.yyyy HH:mm:ss') + ': More than ' + $maxRebootDays + ' days since last reboot. Starting reboot application.'
                Write-Warning $text
                Start-RebootApplication
            } else {
                $text = 'Last boot time: ' + $lastBootTime.ToString('dd.MM.yyyy HH:mm:ss') + ': More than ' + $maxRebootDays + ' days since last reboot. Reboot application disabled.'
                Write-Warning $text
            }
        }
    }
    ...
    Function Test-MissingDrivers {
        ...
        if ($null -ne $devices) {
            ...
            foreach ($device in $devices) {
                ...
                #TODO: Hierfür ein eigenes Log erstellen
                if (-NOT($FileLogLevel -like 'clientlocal')) {
    ...
    Function Test-SCCMHardwareInventoryScan {
        ...
        try {
            #if ($PowerShellVersion -ge 6) {
            $wmi = Get-CimInstance -Namespace root\ccm\invagt -ClassName InventoryActionStatus -ErrorAction SilentlyContinue | Where-Object { $_.InventoryActionID -eq '{00000000-0000-0000-0000-000000000001}' } | Select-Object @{label = 'HWSCAN'; expression = { $_.ConvertToDateTime($_.LastCycleStartedDate) } }
            #}
        ...
        if ($HWScanDate -le $minDate) {
            ...
            if ($fix -eq 'true') {
                ...
                try {
                    ...
                    #if ($PowerShellVersion -ge 6) {
                    $wmi = Get-CimInstance -Namespace root\ccm\invagt -Class InventoryActionStatus -ErrorAction SilentlyContinue | Where-Object { $_.InventoryActionID -eq '{00000000-0000-0000-0000-000000000001}' } | Select-Object @{label = 'HWSCAN'; expression = { $_.ConvertToDateTime($_.LastCycleStartedDate) } }
                    #}
                    #else { $wmi = Get-WmiObject -Namespace root\ccm\invagt -Class InventoryActionStatus | Where-Object { $_.InventoryActionID -eq '{00000000-0000-0000-0000-000000000001}' } | Select-Object @{label = 'HWSCAN'; expression = { $_.ConvertToDateTime($_.LastCycleStartedDate) } } }
    ...
    Function Test-PolicyPlatform {
        ...
        try {
            if (Get-CimInstance -Namespace 'root/Microsoft' -Class '__Namespace' -Filter 'Name = "PolicyPlatform"') {
    ...
    Function Get-Info {
        ...
        $obj = New-Object PSObject -Property @{
            ...
            InstallDate      = Get-SmallDateTime -Date $OS.InstallDate #($OS.ConvertToDateTime($OS.InstallDate))
    ...
    Function Get-XMLConfigLoggingSeverity {

        if ($config) {
            $obj = $Xml.Configuration.Log | Where-Object { $_.Name -like 'File' } | Select-Object -ExpandProperty 'Severity'
        }
    ...
    Function Get-SmallDateTime {
        ...
        if ($null -ne $Date) {
            if ($UTC -eq 'utc') {
                ...
                #$obj = ($Date).ToString('dd-MM-yyyy HH:mm:ss')
                $obj = ($Date).ToString('dd.MM.yyyy HH:mm:ss')
            }
        } else {
            $obj = Get-DateTime
    ...
    Function Update-LogFile {
        ...
        } elseif ($Mode -eq 'ClientInstallFailed') {
            Out-LogFile -Xml $xml -Text $text -Mode $Mode -Severity 1
        } else {
            Out-LogFile -Xml $xml -Text $text -Severity 1
        }

        if ($null -ne $Log.ClientInstalledReason -or $null -ne $Log.ClientInstalled) {
            Out-LogFile -Xml $xml -Text $text -Mode 'ClientReInstalled' -Severity 3
        }
    ...
    # Create a DataTable to store all changes to log files to be processed later.
    # This to prevent false positives to remediate the next time script runs if error is already remediated.
...
Process {
    ...
    If (-NOT ([Security.Principal.WindowsPrincipal] [Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole] 'Administrator') -and $Debug -ne $True) {
    ...
    function Invoke-DateTimeParsing {
        [CmdletBinding()]
        param (
            [Parameter()]
            [String] $DateToParse,
        ...
        $Culture = [System.Globalization.CultureInfo]::InvariantCulture
        $inputFormats = @('MM/dd/yyyy HH:mm:ss', 'dd/MM/yyyy HH:mm:ss', 'dd.MM.yyyy HH:mm:ss')
        $Formats = 'MM/dd/yyyy HH:mm:ss', 'dd/MM/yyyy HH:mm:ss', 'dd.MM.yyyy HH:mm:ss'

        $dateTime = [DateTime]::ParseExact($DateToParse, $Formats, $Culture, [System.Globalization.DateTimeStyles]::None)

        $dateTime = $null

        # Try parsing the date using each format
        foreach ($format in $inputFormats) {
            if ([DateTime]::TryParseExact($DateToParse, $format, $Culture, [System.Globalization.DateTimeStyles]::None, [ref]$dateTime)) {
                # If successful, format and output the date in the desired format
                $formattedDate = $dateTime.ToString($outputFormat, $culture)

                #Write-Output "Original: $date -> Formatted: $formattedDate"
                return $dateTime
    ...
    try {
        ...
        try {
            $Format = 'dd.MM.yyyy HH:mm:ss'
            $LastRun = [DateTime]::ParseExact($LastRunEntry, $Format, $Culture)
        } catch {
            $Format = 'MM/dd/yyyy HH:mm:ss'
            $LastRun = [DateTime]::ParseExact($LastRunEntry, $Format, $Culture)
        }

        #$LastRun = Invoke-DateTimeParsing -DateToParse $LastRunEntry
        #$LastRun = Get-Date -Date $LastRunEntry -Format 'dd.MM.yyyy HH:mm:ss'
    ...
    #$LastRun = Get-Date -Date $LastRunEntry #-Format 'dd.MM.yyyy HH:mm:ss'
    #Write-Output "Script last ran: $(Get-Date -Date $LastRunEntry -Format 'dd.MM.yyyy HH:mm:ss')"
    Write-Output "Script last ran: $($LastRun.ToString('dd.MM.yyyy HH:mm:ss', $Culture))"
    ...
    #TODO: Checks von Get-CCMAStatus.ps1 einbauen
    ...
    if ($WMI -like 'True') {
        ...
        if ((Test-WMI -log $Log) -eq $true) {
            ...
            Out-LogFile -Xml $Xml -Text 'Corrupt WMI.' -Severity 3
            Out-LogFile -Xml $Xml -Text 'Corrupt WMI.' -Mode 'ClientReInstalled' -Severity 3
        }
    }
    ...
    if ( ($RefreshComplianceState -like 'True') -or ($RefreshComplianceState -ge 1)) {
        ...
        Test-RefreshComplianceState -Days $RefreshComplianceStateDays -RegistryKey $Script:CHRegistryPath -log $Log
    }
    ...
    if ((Test-ClientVersion -Log $log) -eq $true) {
        if ($clientAutoUpgrade -like 'true') {
            $reinstall = $true
            New-ClientInstalledReason -Log $Log -Message 'Below minimum version.'

            Out-LogFile -Xml $Xml -Text 'Below minimum version.' -Severity 2
        }
    }
