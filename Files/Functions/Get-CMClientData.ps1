...
function Get-CMClientLastInstalledUpdates {
    ...
    $KBs | Select-Object KB, Date, Title
...
function Get-CMClientFreeDiskSpace {

    (([wmi]"root\cimv2:Win32_logicalDisk.DeviceID='C:'").FreeSpace / 1GB).ToString("N2") + "GB"
}

function Get-CMClientLoggedOnUser {

    "User:" + (Get-WMIObject -query "SELECT * FROM win32_Process WHERE Name ='explorer.exe'" | ForEach-Object {  $owner = $_.GetOwner();  $_ | Add-Member -MemberType "Noteproperty" -name "Owner" -value $("{0}\{1}" -f $owner.Domain, $owner.User) -passthru }).Owner
...
Function Get-CMClientPendingReboot {
    <#
.SYNOPSIS
